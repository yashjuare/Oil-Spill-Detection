...
def main():
    ...
    sample_option = st.sidebar.selectbox(
        ...
        options=["Select any one image sample", "Class1_sample1.jpg", "Class1_sample2.jpg", "Class1_sample3.jpg",
    ...
    if file_upload:
        ...
        st.image(data, caption="Uploaded Image", width=300)
    elif sample_option:
        if sample_option=="Select any one image sample":
            ...
            st.session_state["image_data"] = np.array(data)
            st.image(data, caption=f"Sample: {sample_option}", width=300)
    else:
    ...
    if st.button("View Mask Image"):
        if "image_data" in st.session_state:
            ...
            st.image(mask_img, caption="Detected Mask (YOLOv8)", width=300)
